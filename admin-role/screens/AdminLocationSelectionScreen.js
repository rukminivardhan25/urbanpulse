...
import { verifyOTP, updateAdminLocation } from '../utils/api';
import { useAdminAuth } from '../contexts/AdminAuthContext';
import { searchPlaces, reverseGeocode, debounce } from '../utils/placeSearch';

export default function AdminLocationSelectionScreen() {
  ...
  const handleSaveLocation = async () => {
    if ((!selectedPlace && (!latitude || !longitude)) || !state || !city) {
      Alert.alert('Error', 'Please select a valid location with city and state');
      return;
    }

    const { phone, otp, name, adminId } = route.params || {};
    ...
    try {
      ...
      // If this is during signup flow (has phone and otp), verify OTP with location
      if (phone && otp && name) {
        // Verify OTP with location data (creates admin account)
        const result = await verifyOTP(phone, otp, name, locationData);
        
        // Verify that admin was created successfully
        if (!result.admin) {
        ...
        // If admin already exists, just update location
        await updateAdminLocation(locationData);
      ...
      if (errorMessage.includes('already exists') || errorMessage.includes('Account already exists')) {
        ...
        Alert.alert('OTP Expired', 'OTP has expired. Please go back and request a new OTP.');
      ...
      <View style={styles.content}>
        <View style={styles.searchSection}>
          ...
          <View style={styles.searchContainer}>
            ...
            />
            {isSearching && (
              <ActivityIndicator size="small" color={theme.colors.admin} style={styles.searchLoader} />
            )}
          </View>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <Map size={24} color={theme.colors.admin} />
  ...
  const generateMapHTML = () => {
    ...
    const primaryColor = theme.colors.admin;
    ...
    >
      <View style={styles.mapModalContainer}>
        ...
        <View style={styles.mapActions}>
          ...
          >
            {mapLoading ? (
              <ActivityIndicator size="small" color={theme.colors.admin} />
            ) : (
              <NavigationIcon size={20} color={theme.colors.admin} />
            )}
